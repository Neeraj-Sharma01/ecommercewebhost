...
export const getProducteController = async(req,res) => {
    try {
...
export const getSingleProducteController = async(req,res) => {
    try {
      ...
      .populate("category");
        ...
        res.status(500).send({
            success:false,
            message:'Error gettinng Single Product',
            error
        })
...
export const productListController = async (req,res)=> {
  try {
    ...
    res.status(400).send({
      success:false,
      message:"Error While lst to Paginaton",
      error
    })
...
try {
  ...
  res.status(400).send({
    success:false,
    message:"Error in Simillar Product",
    error
  })
}
}
